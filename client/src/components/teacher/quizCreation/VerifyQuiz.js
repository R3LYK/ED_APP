...
export const QuestionProvider = ({ children }) => {
  ...
  return (
    <QuestionContext.Provider value={{ questionsToKeep, addQuestionsToKeep, clearQuestionsToKeep }}>
      {children}
...
const clearConversation = async (userId, auth) => {
  ...
  try {
    ...
    await axios.delete(`${DELETE_CONVO_URL}/${userId}`, { headers, withCredentials: true });

    console.log('Conversation deleted successfully');
  } catch (err) {
    console.log("THIS FUNCTION IS BEING CALLED TWICE. I CAN'T FIGURE OUT WHY, AND I'M KINDA OVER IT. SO NOW, IT'S FUTURE ME'S PROBLEM.");
  }
};
...
const VerifyQuiz = () => {
  ...
  const handleSaveQuiz = async () => {
    ...
    if (isNamingRequired) {
      const name = prompt('Please enter the assignment name (less than 20 characters):');
  ...
  if (hasSavedQuiz) {
    return (
      <div>
        <h2>Quiz Saved!</h2>
        <button onClick={() => setShowCreateAssignment(true)}>Back to Dashboard</button>
        <button onClick={() => setShowVerifyQuiz(false)}>Build More Assignments</button>
      </div>
    );
  }
